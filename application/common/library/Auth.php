...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'money', 'email', 'bonus', 'name'];
     ...
     * @return boolean
     */
    public function register($username, $password, $email = '', $mobile = '', $extend = [], $data = [])
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        $checkUserName = User::where('username', $username)->where('origin', $extend['origin'])->find();
        ...
        // 注册赠送活动
        $activity = \app\common\model\Activity::where('name', 'register_bonus')->find();
        $reward_data = [];
        if($activity && $activity->status == 1){
            if(isset($activity->config) && $activity->config){

                $activity_money = $activity->config['money']; // 注册奖励金额
                $activity_typing_amount_limit = $activity->config['typing_amount_limit']; // 注册打码量
                
                // 首充数据
                $reward_data = [
                    'register_bonus' => [
                        'money'                 => $activity_money, // 首充奖励金额
                        'typing_amount_limit'   => $activity_typing_amount_limit,
        ...
        try {
            ...
            // 插入奖励记录
            if($reward_data){
                $this->_user::insertLog($this->_user, $reward_data);
            }
            ...
            \think\Log::record($e->getMessage(), 'register2');
            Db::rollback();
            return false;
        }
        return true;
    ...
    public function login($account, $password, $origin, $area_code = '+55')
    {
        ...
        if (!$user) {
            $this->setError(__('帐号不正确'));
            return false;
        }

        if ($user->status != 'normal') {
            $this->setError(__('账户已经被锁定'));
            return false;
        }

        if ($user->loginfailure >= 10 && time() - $user->loginfailuretime < 86400) {
            $this->setError(__('请于 1 天后重试'));
            return false;
        }
