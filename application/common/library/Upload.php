...
class Upload
{
     ...
     * 自定义到指定上传目录
     */
    public function uploadCustom($savekey = null)
    {
        if (empty($this->file)) {
            throw new UploadException(__('No file upload or server upload limit exceeded'));
        }
        ...
        $savekey = $savekey ? $this->getSavekey($savekey) : $this->getSavekey();
        $savekey = '/' . ltrim($savekey, '/');
        $uploadDir = substr($savekey, 0, strripos($savekey, '/') + 1);
        $fileName = substr($savekey, strripos($savekey, '/') + 1);
        
        // 默认目录file
        $defaultDir = config('upload.rootpath');
        
        //获取根目录 路径
        $destDir = dirname(ROOT_PATH) . '/' . $defaultDir . str_replace('/', DS, $uploadDir); 
        // dd($destDir);
